...
def split_csv_test_validation_pool(data_csv_path=DATA_MAIN_CSV_PATH, test_size=TEST_SPLIT_SIZE, val_size=VALIDATION_SPLIT_SIZE, random_state=INITIAL_DATA_SPLIT_SEED, base_data_split_save_path=BASE_DATA_SPLIT_CSV_PATH):
    ...
    trainval_df, test_df = train_test_split(data_df, test_size=test_size, random_state=random_state, stratify=data_df[DataSplitColNames.LABEL])

    pool_df, val_df = train_test_split(trainval_df, test_size=val_size, random_state=random_state, stratify=trainval_df[DataSplitColNames.LABEL])
...
def split_csv_initial_training_set(base_data_split_csv_path=BASE_DATA_SPLIT_CSV_PATH, initial_training_size=1000, random_state=INITIAL_DATA_SPLIT_SEED):
    ...
    number_of_classes = df[DataSplitColNames.LABEL].nunique()

    initial_training_split_indices = df[df[DataSplitColNames.BASE_SPLIT]==BaseSplitNames.POOL].groupby(DataSplitColNames.LABEL, group_keys=False).sample(initial_training_size // number_of_classes, random_state=random_state).index
